...
async function calcHash(options) {
  ...
  const texts = await Promise.all(options.files.map(async file => await fs.readFile(file, 'utf8')));
...
function makeCtx(options, urls) {
  ...
  const codepoints = _.fromPairs(_.map(options.codepoints, (codepoint, name) => [name, codepoint.toString(16)]));
